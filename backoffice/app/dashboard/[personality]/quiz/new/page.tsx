...
export default function NewQuizPage({ params }: { params: Promise<{ personality: string }> }) {
  ...
  useEffect(() => {
    params.then(p => setPersonality(p.personality as PersonalityType));
  }, [params]);
    ...
    <div className="container mx-auto p-6 max-w-4xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">
          {personalityInfo.emoji} {personalityInfo.name} 퀴즈 추가
        </h1>
        <p className="text-gray-600">{personalityInfo.nameKo} 성향의 새 퀴즈를 작성합니다.</p>
      </div>

      {personality && <QuizContentForm personality={personality} />}
    </div>
  );
}
